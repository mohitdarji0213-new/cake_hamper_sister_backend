...
exports.createOrder = async (req, res) => {
  ...
  await sendEmail({
    to: customer.email,
    subject: `Order Confirmed - ${order.orderNumber} | Cake Hamper Sisters`,
    html: `<h2>Namaste ${customer.name}!</h2><p>Aapka order place ho gaya hai.</p><p>Order Number: <strong>${order.orderNumber}</strong></p><ul>${itemsList}</ul><p>Total: <strong>₹${totalAmount}</strong></p><p>Payment: ${paymentMethod}</p><p>- Garima Bothra, Cake Hamper Sisters</p>`
  });

  // Notify admin (email) — badge/browser notification handled by /api/notifications polling on the frontend
  await sendEmail({
